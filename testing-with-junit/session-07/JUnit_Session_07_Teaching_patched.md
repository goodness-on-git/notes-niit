...
**Note on ordering:** here the stub (`when(...)`) is written *before*
`UserService service = new UserService(repository);`. That doesn't
match the "four stages" order from Session 6 (arrange dependency →
execute → assert → verify) line-for-line. That's fine — setup lines
(creating the mock, stubbing it, constructing the service) can happen
in any order relative to each other, because nothing runs yet. What
matters is that all setup happens *before* `service.getUserName(10)`
executes. If a student asks why it "still works," that's the answer:
order among setup lines doesn't matter, only setup-before-execution
does.
...
**Caveat (mention, don't explain):** this only works cleanly when the
class under test has one obvious constructor whose parameter matches
the mock's type — which is true for every class in this session. If a
student's class ever has more than one constructor, or a field whose
type doesn't match any declared `@Mock`, `@InjectMocks` can silently
fail to wire the mock in. The service then runs against a real (empty)
dependency instead of the mock, which shows up as Error 3 in §16 —
but for a reason that looks like nothing is wrong with their test
code. Worth one sentence of warning now so it isn't a mystery later;
the mechanism itself stays deferred.
...
Then write a second test for the opposite case:

> Write a Mockito test where the mocked repository returns `false`,
> asserting that the service returns `false`, and verifying the
> repository was called with the correct account number.

This mirrors the succeeds/fails pairing from Session 6 (§5) —
mocking a "yes" case only demonstrates half of what a mock is for.
Students should leave this exercise able to control a dependency
toward *either* outcome, not just the happy path.
